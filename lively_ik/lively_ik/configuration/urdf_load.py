...
from lively_ik.configuration.kdl_parser_py import treeFromFile, treeFromString
...
def convertToArm(urdf_robot, startJoint, endJoint, fixedJoint, Debug=False):
    ...
    for j in joints:
        ...
        if j.name == endJoint:
            e = j
    if s == []:
        raise ValueError('startJoint: {} not found in joint list!  Please check to make sure startJoint is a joint found in' \
                         'the URDF and is spelled correctly'.format(startJoint))
    if e == []:
        raise ValueError('endJoint: {} not found in joint list!  Please check to make sure endJoint is a joint found in' \
                         'the URDF and is spelled correctly'.format(endJoint).format(startJoint))
    ...
    if not fixedJoint == None:
        ...
        for j in joints:
            if j.name == fixedJoint:
                ...
                displacements.append(tuple(currJoint.origin.xyz))
        if currJoint == []:
            raise Exception('fixed_ee_joint: {} not found!'.format(fixedJoint))
...
def findNextJoint(joints, child):
    for j in joints:
        if j.parent == child:
            return j
    raise Exception('joint with matching parent link: {} not found!'.format(child))
