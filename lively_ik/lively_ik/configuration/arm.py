...
from lively_ik.configuration.robot_function import RobotFunction
...
def rotMatrix(axis, s, c):
    if axis=="Z" or axis=="z":
        ...
        print("Unsupported Axis: {0}".format(axis))
...
class Arm(RobotFunction):
    def __init__(self, axes=[], displacements=[], rotOffsets=None, dispOffset=(0,0,0), name="Arm",rep="angle"):
        ...
        RobotFunction.__init__(self, _nvars=len(self.axes) * self.varsPerJoint, _npoints=len(self.axes) + 1, _name=name)
        self.noZ = noZ
        if self.rep == "angle":
