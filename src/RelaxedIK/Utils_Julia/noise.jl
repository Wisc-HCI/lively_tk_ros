...
function NoiseGenerator(arm_position_scale, arm_rotation_scale, base_scale, dc_scale,
                        arm_position_freq,  arm_rotation_freq,  base_freq,  dc_freq)

    num_chains = length(arm_position_scale)
    ...
    # Remove in final
    n = NoiseGenerator(num_chains, num_dc, time,
                       arm_noise, dc_noise, base_noise,
                       arm_position_freq, arm_rotation_freq, dc_freq, base_freq,
                       arm_position_scale, arm_rotation_scale, dc_scale, base_scale,
                       arm_seed, dc_seed, base_seed,
                       global_seed)
...
function update!(noisegen, time, bias)
    ...
    for i=1:noisegen.num_chains
        if noisegen.arm_position_scale[i] > 0.0 || noisegen.arm_rotation_scale[i] > 0.0
            noisegen.arm_noise[i].position = noise3D(noisegen.time,noisegen.arm_seed[i].position,noisegen.arm_position_freq[i]) * noisegen.arm_position_scale[i] * limit(time,noisegen.global_seed) .* bias
    ...
    if noisegen.base_scale > 0.0
        noisegen.base_noise.position = noise3D(noisegen.time,noisegen.base_seed.position,base_freq) * noisegen.base_scale .* bias
