...
function update!(noisegen, time, bias)
    ...
    for i=1:noisegen.num_chains
        if noisegen.arm_scale[i] > 0.0
            noisegen.arm_noise[i].position = noise3D(noisegen.time,noisegen.arm_seed[i].position) * noisegen.arm_scale[i] * limit(time,noisegen.global_seed) .* bias
            noisegen.arm_noise[i].rotation = noise3D(noisegen.time,noisegen.arm_seed[i].rotation) * noisegen.arm_scale[i] * 0.3
        end
    end
    # Handle Direct Control
    for i=1:noisegen.num_dc
        if noisegen.dc_scale[i] > 0.0 && noisegen.dc_weight[i] > 0
            noisegen.dc_noise[i] = noise1D(noisegen.time,noisegen.dc_seed[i]) * noisegen.dc_scale[i]
        end
    end
    # Handle fixed frame noise
    if noisegen.base_scale > 0.0
        noisegen.base_noise.position = noise3D(noisegen.time,noisegen.base_seed.position) * noisegen.base_scale .* bias
